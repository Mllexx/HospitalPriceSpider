...
app.get('/api/csv-to-db', async (req, res) => {
    ...
    for(const $a of $k){
        ...
        //if filename is not available skip
        var $filename = $a.savedRepoTableName.trim();
        if($filename===''){
            /* @TODO: Update DB or move file to another folder for files with issues */
            continue;//stop processing and move on to the next item in the list
        }
        //Check itemColumnName
        var $itemColumnName = $a.itemColumnName.trim();
        if($itemColumnName===''){
            //attempt to get itemNameColumn using regex
            continue;
        }
        //if price column is null attempt to get it
        var $avgPriceColumnName = $a.avgPriceColumnName.trim();
        if($avgPriceColumnName===''){
            //attempt to get price column using regex
            continue;
        }
        //console.log($a);
        var $d = csvToJsonService.getJsonFromCsv($csvPath + '\\' + $filename+".csv");
        //var $d = getFileData();
        console.log($d);
        //var $content = fs.readFileSync($csvPath + '\\' + $filename+".csv", 'utf8');
        //console.log($content);
